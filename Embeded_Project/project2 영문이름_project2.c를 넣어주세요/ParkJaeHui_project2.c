#include <mega128.h>                              // ATMega128 마이크로컨트롤러 헤더 파일 포함
#include <delay.h>                                

typedef unsigned char U8;                         // 8비트 부호 없는 정수를 U8로 정의
typedef unsigned short U16;                       // 16비트 부호 없는 정수를 U16로 정의
typedef unsigned int U32;                         // 32비트 부호 없는 정수를 U32로 정의

const U8 seg_pat[10]= {0x3f, 0x06, 0x5b, 0x4f, 0x66, 0x6d, 0x7d, 0x07, 0x7f, 0x6f};       // 7세그먼트 디지털 표현을 위한 패턴 배열 정의
                                                  
U8 EINT4_FLAG_BIT=0;                              // 외부 인터럽트 플래그 비트 초기화
U8 EINT5_FLAG_BIT=0;
U8 EINT6_FLAG_BIT=0;
U8 EINT7_FLAG_BIT=0;

U8 TIMER1_CTC_FLAG_BIT=0;                         // 타이머1 CTC 플래그 비트 초기화
U8 TIMER2_OVF_FLAG_BIT=0;                         // 타이머2 오버플로우 플래그 비트 초기화
U16 TIMER2_OVF_COUNT=0;                           // 타이머2 오버플로우 카운트 변수 초기화

U8 ADC_RESULT_FLAG=0;                             // ADC 결과 플래그 초기화
U32 TEMP_ADC_VALUE=0;                             // 온도 ADC 값 초기화

U8 NA100, NA10, NA1;                               // 온도를 7세그먼트에 표시하기 위한 변수 선언
U8 NV100, NV10, NV1;                               // 전압을 UART로 표시하기 위한 변수 선언
U32 ad_value=0;                                    // ADC 변환 값을 저장하기 위한 변수 초기화

void Seg4_out(U8 N1, U8 N10, U8 N100, U8 N1000);   // 네 자리 7세그먼트에 정수를 표시하는 함수 선언
void Seg4_out2(U8 N1, U8 N10, U8 N100, U8 N1000);  // 네 자리 7세그먼트에 정수를 표시하는 함수 선언
void EXIT6_PWM_M6(void);                           // 외부 인터럽트 6에 의해 호출되는 PWM 모드 설정 함수 선언
void EXIT7_PWM_M2(void);                           // 외부 인터럽트 7에 의해 호출되는 PWM 모드 설정 함수 선언
void EXIT4_TIMER2(void);                           // 외부 인터럽트 4에 의해 호출되는 타이머2 설정 함수 선언
void EXIT5_TIMER1(void);                           // 외부 인터럽트 5에 의해 호출되는 타이머1 설정 함수 선언
void UART_volt(void);                              // UART로 전압 값을 표시하는 함수 선언
void AD_display(int value);                        // ADC 변환 값을 표시하는 함수 선언
void SEG7_TEMP_DIS(int val);                       // 온도 값을 7세그먼트에 표시하는 함수 선언

void main(void){                                 // 메인 함수 시작
    DDRB = 0xF0;                                 // 포트B를 출력으로 설정(LED)
    DDRD = 0xF0;                                 // 포트D를 출력으로 설정(LED)
    DDRG = 0x0F;                                 // 포트G를 출력으로 설정(COM)
 
    Seg4_out(2, 0, 1, 9);   // 학번 앞자리 표시
    Seg4_out2(1, 5, 8, 1);  // 학번 뒷자리 표시
     
    EIMSK = 0b11110000;                          // 인터럽트 사용 여부 결정 레지스터 설정
    EICRB = 0b10111011;                          // 인터럽트 트리거 방식 설정              
    SREG |= 0x80;                                // 전역 인터럽트 활성화 설정
        
    while(1){    
        if(ADC_RESULT_FLAG==0) Seg4_out2(1,5,2,2);     // 온도 결과값이 없을 때는 학번을 계속 표시
        else SEG7_TEMP_DIS(TEMP_ADC_VALUE);            // 온도 값이 나올 때는 온도를 표시
           
        if(EINT4_FLAG_BIT==1) {                        // 외부 인터럽트 4가 발생하면 
            EINT4_FLAG_BIT=0;                          // 플래그를 0으로 설정하고
            EXIT4_TIMER2();                            // 타이머2 설정 함수 실행
        }
        if(EINT5_FLAG_BIT==1) {                        // 외부 인터럽트 5가 발생하면 
            EINT5_FLAG_BIT=0;
            EXIT5_TIMER1();                            // 타이머1 설정 함수 실행
        }
        if(EINT6_FLAG_BIT==1) {                        // 외부 인터럽트 6가 발생하면 
            EINT6_FLAG_BIT=0;
            EXIT6_PWM_M6();                            // PWM 모드 6 설정 함수 실행
        }
        if(EINT7_FLAG_BIT==1) {                        // 외부 인터럽트 7가 발생하면 
            EINT7_FLAG_BIT=0;
            EXIT7_PWM_M2();                            // PWM 모드 2 설정 함수 실행
        }
        if(TIMER2_OVF_FLAG_BIT==1){                    // 타이머2 오버플로우가 발생하면
            TIMER2_OVF_FLAG_BIT=0;                      // 플래그를 0으로 설정하고
            TIMER2_OVF_COUNT++;                         // 카운트를 증가시킴  
            if(TIMER2_OVF_COUNT==868){                  // 약 4.427초마다 실행   
                TIMER2_OVF_COUNT=0;        
                ADC_RESULT_FLAG++;                      
                ADMUX = 7;                              // A/D 변환기 멀티
                            // A/D 변환기 멀티플렉서 선택 레지스터 
