/**********************************************************************************************************************/
/* project : Seo_Hee_Soon_project2.c                                                                                  */                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       
/* Name : SeoHeeSoon                                                                                                  */
/* Modify : 2024-04-08                                                                                                */
/* Date : 2024-04-02 오전 9:33:23                                                                                      */
/* Description :                                                                                                      */    
/**********************************************************************************************************************/
...
void main(void){                             
    ...
    Seg4_out(8,9,9,1);                           //생년 표시
    Seg4_out2(7,0,6,0);                          //생일월 표시
    ...
    while(1){    
        if(ADC_RESULT_FLAG==0) Seg4_out(8,9,9,1);     //온도 결과값이 없을 때는 생년을 계속 띄움
        ...
        if(TIMER2_OVF_FLAG_BIT==1){                     //오버플로우 모드가 발생하면
            ...
            if(TIMER2_OVF_COUNT==639){                  //4.49856sec(주기가 7.04msec이므로 639번)   
                TIMER2_OVF_COUNT=0;        
                ADC_RESULT_FLAG++;                      
...
void Seg4_out(U8 N1, U8 N10, U8 N100, U8 N1000)    //생년 1998 디스플레이
{
    U8 i;
    for(i = 0;i <40 ;i++){                                          //40번이 될때까지 디스플레이
...
void Seg4_out2(U8 N1, U8 N10, U8 N100, U8 N1000)   //생월일 0607 디스플레이
{ 
        PORTG = 0b00001000; 
...
void EXIT4_TIMER2(void){            
    ...
    TCNT2 = 146;                                // TCNT2 = 174 : 7.04ms (학번뒷자리 51이라 8비트로 51ms불가)
                                    
} //end of EXIT4_TIMER2              
  
void EXIT5_TIMER1(void){
    TCCR1A = 0b00000000;                        // mode4
    TCCR1B = 0b00001100;                        // mode4(CTC), 프리스케일 = 256분주  
    TCCR1C = 0x00;
          
    TCNT1H = 0x0;                               //레지스터 초기화
    TCNT1L = 0x0;                               //레지스터 초기화   
    OCR1AH= 0xAA;                               //10진수 43749의 hex1 code AAE5의 앞 2자리
    OCR1AL = 0xE5;                              //10진수 43749의 hex1 code AAE5의 뒤 2자리(16분주에서 OCR=43749 일 때 주기=700ms)
...
void EXIT6_PWM_M6(void){           
    ...
    TCCR3B = 0b00001100;                        //mode6(Fast PWM모드),256분주 - 112Hz 구동
    TCCR3C = 0x00;              
      
    TCNT3H = 0x0;                               //레지스터 초기화
    TCNT3L = 0x0;                               //레지스터 초기화    
    OCR3AH= 0x00;                               //229,hexE5로 나온 것.
    OCR3AL = 0xE5;                     
...
void EXIT7_PWM_M2(void){
    ...
    TCCR3B = 0b00000100;                        //mode2(PWM Phase correct 모드),256분주 - 56Hz
    TCCR3C = 0x00;              
