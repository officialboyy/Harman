    ...
    while(1){
        ...
        if(TIMER2_OVF_FLAG_BIT==1){                     //오버플로우 모드가 발생하면
            ...
            if(TIMER2_OVF_COUNT==1730) {    
                ...
                delay_ms(2);  // 왜 delay 함수를 사용했을까. conversion완료비트 확인을 위한 while을 사용하지 않고??????           
                ad_val = (int)ADCL + ((int)ADCH << 8); // A/D값 읽기
                TIMER2_OVF_COUNT=0;
                Adc_Flag++;
                if(Adc_Flag==10){ 
                  Adc_Flag%=10;                  // 10회동안만 온도 표시    
...
interrupt [EXT_INT4] void external_int4(void){   
    SREG &= 0x7F;       // Nesting 허용안함 All Interrupt disable                               
    EINT4_FLAG_BIT=1;                            
    SREG |= 0x80;                                
}                                                      
...
interrupt [TIM2_OVF] void TIMER2_OVF_int(void){ 
    SREG &= 0x7F;       // Nesting 허용안함 All Interrupt disable  
    TIMER2_OVF_FLAG_BIT++;                                //초기값을 인터럽트 안에도 써줘야함 
    SREG&=0x7F;       
}
