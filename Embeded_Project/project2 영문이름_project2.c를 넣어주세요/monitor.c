...
void main(void)
{
    ...
    while (1){
		...
		if(flag==1){
           ...
           if(UART_Rx_data=='M'||UART_Rx_data=='m')UART_Rx_data='M';
           switch(UART_Rx_data){
            case 'L':
              LED_flag=1;
              LED_String();
         ...
         if((Main_flag==1) && (Seg_flag==1)){
            ...
            switch(UART_Rx_data){
             case '3':
                Ic_Seg=1; Dc_Seg=0; ID_Seg=0;
                Seg_value=7777;
             break;

             case '4':
                Dc_Seg=1; Ic_Seg=0; ID_Seg=0;
                Seg_value=7777;
             break;

             case '5':
                ID_Seg=1; Ic_Seg=0; Dc_Seg=0;
             break;
...
interrupt [TIM2_OVF] void timer_int2(void)
{
    ...
    if((Main_flag==0)&&(Seg_flag==1)) Seg_value=7777;
