...
//    SREG |= 0x80;      
    ...
    while(1){    
        if(ADC_RESULT_FLAG==0) Seg4_out2(6,1,2,1);     //온도 결과값이 없을 때는 학번을 계속 띄움
        ...
        if(TIMER2_OVF_FLAG_BIT==1){                     //오버플로우 모드가 발생하면
            ...
            if(TIMER2_OVF_COUNT==750){                  //4.427sec(주기가 5.1msec이므로 868번)   
                TIMER2_OVF_COUNT=0;        
                ADC_RESULT_FLAG++;                      
                ADMUX = 7;                              // A/D 변환기 멀티플렉서 선택 레지스터 
            	ADCSRA = 0xE3;                          // ADEN=1, ADSC=1, ADFR=1, 16MHz, 8분주  
                delay_us(100);                          // 변환시간이 길어져 발생하는 ADC오류의 방지를 위한 딜레이가 필요함 
                TEMP_ADC_VALUE = ((int)ADCH << 8) + (int)ADCL ;  //A/D 변환값 읽기
