...
#define CLK_HIGH    PORTD |= 0x01   // PD.0=1             1ºÒ ¿Â
#define CLK_LOW     PORTD &= 0xFE   // PD.0=0             1ºÒ ¿ÀÇÁ

#define DAT_HIGH    PORTD |= 0x02   // PD.1=1             2ºÒ ¿Â
#define DAT_LOW     PORTD &= 0xFD   // PD.1=0             2ºÒ ¿ÀÇÁ
    
#define SCL_OUT     DDRD |= 0x01    // PD.0=1             SCL Æ÷Æ® D Ãâ·Â
#define SDA_IN      DDRD &= 0xFD    // PD.1=0             SDA ÀÔ·Â
#define SDA_OUT        DDRD |= 0x02    // PD.1=1             SDA Ãâ·Â
#define N 5
typedef unsigned char U8;

U8 DEV_ADD_W = 0xA0;                 //DEV ADD  ¾²±â 1010 0000
U8 DEV_ADD_R = 0xA1;                 //DEV ADD  ÀÐ±â 1010 0001
U8 IIC_ADD = 0x55;                      //¾îµå·¹½º 0
U8 IIC_DAT = 0x55;                      // µ¥ÀÌÅÍ 0
U8 READ_DATA = 0x00;                    // ¸®µåµ¥ÀÌÅÍ 0
...
void IIC_START(void);                // IIC ½ºÅ¸Æ®
void IIC_STOP(void);                 // IIC ½ºÅ¾
//void ACK_write(void);
void no_ACK(void);
void ACK_read(void);
void Process_8data(U8 value);        // ÇÁ·Î¼¼½º 8ºñÆ® µ¥ÀÌÅÍ
...
void Byte_write(void){        // Bytewrite ÇÔ¼ö 
    IIC_START();              // start
...
void Page_write(void){        // Bytewrite ÇÔ¼ö 
    U8 i=0;
    IIC_START();              // start
    Process_8data(DEV_ADD_W); // device address 7bit¿Í write 
    ACK_read();               // ack
...
void IIC_START(void)                           //START¿¡ ÇÏÀÌÇÏÀÌ ·Î¿ì·Î¿ì ½ÃÀÛ
{
    DAT_HIGH;
    delay_us(6);  
  CLK_HIGH;
...
void IIC_STOP(void)                            //STOP´Â ·Î¿ìÇÏÀÌ ÇÏÀÌ·Î¿ì ³¡
{
    DAT_LOW;
    delay_us(6);  
    CLK_HIGH;
...
void Process_8data(U8 value)                   //8data W
{
    ...
    {
        if((value_buffer&0x80)==0x80)          //¸¸¾à 1000 0000ÀÌ¸é dat high
            DAT_HIGH;
        else                                   //¾Æ´Ï¸é data low
            DAT_LOW;
        ...
        delay_us(6);  
        CLK_HIGH;  
        delay_us(6);         //Å¬·° ÇÏÀÌ °¬´Ù°¡ ´Ù½Ã ·Î¿ì  8¹ø ¹Ýº¹
        CLK_LOW; 
        delay_us(6);         
           value_buffer<<=1;
    }
...
void ACK_read(void){
    U8 i = 0;                                // ACK È®ÀÎ¹Þ±âÀ§ÇÑ ¹Ýº¹¹® º¯¼ö

    SDA_IN;                                  // µ¥ÀÌÅÍ ÀÔ·Â
    CLK_HIGH;                                // ACK ½ÅÈ£ ¹Þ±âÁØºñ
    for(i=10;i>0;i--){                       // 10¹ø ¹Ýº¹Áß  ACK ¹ÞÀ¸¸é ¹Ýº¹¹® ºüÁ® ³ª°¨
        if((PIND & 0x02) == 0x00){i=1;
        PORTC=0xFE;
        }
        else PORTC=0x0F;                     // NACK ¹Þ¾Ò´Ù¸é LED ´Ù ÄÑÁü                           
    }//end of for
    delay_us(6);
    CLK_LOW;        
    SDA_OUT;                                 //µ¥ÀÌÅÍ Ãâ·Â
...
void Random_read(void){      // Randomread ÇÔ¼ö
    IIC_START();             // start
    Process_8data(DEV_ADD_W);// device address 7bit¿Í write 0
    ACK_read();              // ack
    Process_8data(IIC_ADD);  // address 8bits
    ACK_read();              // ack
    IIC_START();             // start
    Process_8data(DEV_ADD_R);// device address 7bit¿Í read 1
    ACK_read();              // ack
    Process_8data_read();    // data 8bits read
    no_ACK();                // nack µ¥ÀÌÅÍ ±×¸¸ ÁÖ±â master->slave
    IIC_STOP();  
//    PORTC=READ_DATA;            // ½ÃÇè¿ëÀ¸·Î READ_DATA È®ÀÎ¿ë  
}//end of Random_read

void no_ACK(void){       // NACK ÇÔ¼ö  master -> slave
    SDA_OUT;  
    DAT_HIGH;    
    CLK_HIGH;  
    delay_us(12);
...
void ACK(void){       // NACK ÇÔ¼ö  master -> slave
    SDA_OUT;  
    DAT_LOW; 
    delay_us(6);     
    CLK_HIGH;  
