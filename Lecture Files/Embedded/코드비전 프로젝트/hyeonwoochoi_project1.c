...
void Page_write()
{
     U8 i=0;
	IIC_START();              // start
    Process_8data(DEV_ADD_W); // device address 7bitì™€ write 
    ACK_read();               // ack
    Process_8data(IIC_ADD);   // address 8bits
    ACK_read();  				// ack
...
void Random_read(void){      // Randomread í•¨ìˆ˜
    IIC_START();             // start
    Process_8data(DEV_ADD_W);// device address 7bitì™€ write 0
    ACK_read();              // ack
    Process_8data(IIC_ADD);  // address 8bits
    ACK_read();              // ack
    IIC_START();             // start
    Process_8data(DEV_ADD_R);// device address 7bitì™€ read 1
    ACK_read();              // ack
    Process_8data_read();    // data 8bits read
    no_ACK();                // nack ë°ì´í„° ê·¸ë§Œ ì£¼ê¸° master->slave
    IIC_STOP();              // stop 
}//end of Random_read



void no_ACK(void){       // NACK í•¨ìˆ˜  master -> slave
    SDA_OUT;
    DAT_HIGH;
    delay_us(5);    
    CLK_HIGH;  
