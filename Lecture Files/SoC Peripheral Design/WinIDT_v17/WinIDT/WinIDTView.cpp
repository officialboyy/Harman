...
#define	TCP_TX_BUF_SIZE		1200

extern int tcpTotalRxSize;
...
void CWinIDTView::OnBnClickedButtonInfoRead()
{
	...
	BYTE sData[TCP_TX_BUF_SIZE];

	gTcpTotalRcvData = 0;
	::ZeroMemory(gTcpGetBuf, sizeof(gTcpGetBuf));

	for(int i=0; i<TCP_TX_BUF_SIZE/48; i++)
	{
		for(int j=0; j<48; j++)
		{
			if(j<24)	sData[i*48+j] = 'A'+j;
			else		sData[i*48+j] = 'a'+j-24;
		}
	}
/*
	CString sText = _T(""), sText2 = _T("");
	for(int i=0; i<TCP_TX_BUF_SIZE/48; i++)
	{
		...
		for(int j=0; j<48; j++)
		{
			sText2.Format(_T("%c"), sData[i*48+j]);
			sText += sText2;
		}
		UpdateDebugMsg(sText);
	}
*/

	tcpTotalRxSize = 0;
	m_Socket[0].Send(sData, TCP_TX_BUF_SIZE);
	UpdateDebugMsg(_T("\r\n send ok ..\r\n"));
}
