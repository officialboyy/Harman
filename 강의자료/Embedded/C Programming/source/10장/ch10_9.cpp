//[프로그램 10-9] 4가지 정렬 알고리즘을 이용해 배열을 오름차순 정렬하기 (ch10-9.cpp)
#include <stdio.h>
...
void select_sort(int a[]) 					// 선택 정렬: 배열명[첨자] 표기법만 이용
{
	...
	for (i=0; i<SIZE-1; i++)
	{
		index = i;							// a[i]를 최소값으로 가정하고 index를 i로 초기화
		for (j=i+1; j<SIZE; j++)
		{			  
			if (a[index] > a[j])			// 현재의 최소값 a[index]보다 a[j]가 더 작다면
				index = j;	 				// index를 새 최소값의 첨자 j로 수정하기 	
		}
		swap(&a[i], &a[index]);				// a[i]와 a[index]를 교환
	}
}

void insert_sort(int *a)					// 삽입 정렬: 포인터 포기법과 배열명[첨자] 표기법 혼용
{
	int i, j, target;
